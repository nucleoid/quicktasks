...
public class QuickTaskResource {
    ...
    public Response get(@PathParam("id") String id, @Context HttpServletRequest request)
    {
        ...
        List<sbt.jira.plugins.quicktasks.entities.QuickTask> tasks = quickTaskManager.findByIssueId(issueId);
        
        List<QuickTask> converted = new ArrayList<QuickTask>();
        
        for(sbt.jira.plugins.quicktasks.entities.QuickTask entityTask : tasks){
        	converted.add(new QuickTask(entityTask));
        }
        request.getSession().setAttribute(QuicktaskViewOptionsFactory.QUICK_TASK_VIEW,
        		QuicktaskViewOptionsFactory.QUICK_TASK_VIEW_ALL);
    ...
    @Path("{id}/{call}")
    @Produces(MediaType.APPLICATION_JSON)
    public Response get(@PathParam("id") String id, @PathParam("call") String call, 
    		@Context HttpServletRequest request)
    {
        ...
        if("incomplete".equals(call)){
        	final Long issueId = Long.parseLong(id);
            List<sbt.jira.plugins.quicktasks.entities.QuickTask> tasks = quickTaskManager.findByIssueIdIncomplete(issueId);
            ...
            request.getSession().setAttribute(QuicktaskViewOptionsFactory.QUICK_TASK_VIEW,
            		QuicktaskViewOptionsFactory.QUICK_TASK_VIEW_INCOMPLETE);
            return Response.ok(converted).build();
        }
        else 
        {
        	String urlType = (String)request.getSession().getAttribute(QuicktaskViewOptionsFactory.QUICK_TASK_VIEW);
            return Response.ok(urlType, MediaType.TEXT_PLAIN).build();
        }
    ...
    public Response post(@PathParam("taskId") int taskId, String position, @Context HttpServletRequest request)
    {
        ...
        String[] positionSplit = position.replaceAll("}", "").replaceAll("\"", "").split("/");
        Integer after = null;
        if(!position.toLowerCase().contains("first"))
        	after = Integer.parseInt(positionSplit[positionSplit.length - 1]);
    ...
    public Response delete(@PathParam("id") String id, @Context HttpServletRequest request)
    {
        ...
        sbt.jira.plugins.quicktasks.entities.QuickTask task = quickTaskManager.findById(taskId);
    ...
    public static final class QuickTask
    {
        ...
        public QuickTask(sbt.jira.plugins.quicktasks.entities.QuickTask quickTask){
        	this();
